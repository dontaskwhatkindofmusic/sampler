...
const tutorialStages=[
 ...
 {title:'play your loop',view:'play',targets:()=>['play','pad'+tutorialKey],text:()=> 'Press Play, then tap '+tutorialKey+' in time with the click. Your hits repeat around the loop. Keep playing until it feels good.',done:()=>playing&&(p.pattern.some(row=>row.includes(tutorialKey))||(p.freeNotes||[]).some(n=>n.key===tutorialKey))},
