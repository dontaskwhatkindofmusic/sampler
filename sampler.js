...
function renderSteps(){for(let page=0;page<4;page++)$('stepPage'+page).setAttribute('aria-pressed',page===stepPage);[...$('steps').children].forEach((b,i)=>{b.hidden=Math.floor(i/8)!==stepPage;b.disabled=i>=p.length;b.setAttribute('aria-pressed',chosen.has(i)||(Math.floor(i/8)===stepPage&&held.has('Digit'+(i%8+1))));b.querySelector('span').textContent=p.pattern[i].join('').toLowerCase()||'—';b.title='Step '+(i+1)+': '+(p.pattern[i].join(', ')||'empty')});$('meter').textContent='· '+(p.length*p.division/4)+' bars of 4/4'}
