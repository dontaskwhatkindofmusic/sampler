...
function render(){for(const k of letters){const b=$('pad'+k),s=p.samples[k];b.dataset.loaded=!!s&&!!buffers[k];b.dataset.audio=s?(buffers[k]?'ready':ready?'unavailable':'loading'):'empty';b.setAttribute('aria-pressed',mobileUI?multiSelected.has(k):selected===k);b.querySelector('small').textContent=s?(s.name+(buffers[k]?'':ready?' · retry':' · loading')):'—';b.title=k+(s?' · '+s.name+(buffers[k]?'':' · audio unavailable; tap to retry'):' · empty')}$('bpm').value=p.bpm;$('length').value=p.length;$('division').value=p.division;renderSteps();editor();renderDelete();$('undoAll').disabled=!undo}
function editor(){const s=p.samples[selected];$('selected').textContent='selected: '+selected+(mobileUI?'':' [←/→]');$('sampleName').value=s?.name||'';$('gain').value=s?.gain??1;$('reverse').checked=s?.reverse||false;$('start').value=s?.start||0;$('end').value=s?.end||0;$('size').textContent=s?((s.blob?.size||0)/1024).toFixed(1)+' KB · '+(buffers[selected]?buffers[selected].duration.toFixed(2)+' s':'audio unavailable'):'empty';for(const id of ['sampleName','gain','reverse','start','end','erase'])$(id).disabled=!s||(['start','end'].includes(id)&&!buffers[selected])}
...
KeyQ:['clearAll','click'],KeyU:['busOpen','click'],KeyO:['restore','click'],KeyY:['fxOpen','click'],Backspace:['clearPattern','click'],Delete:['clearPattern','click']
};
