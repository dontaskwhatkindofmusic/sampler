...
const fresh=()=>({bpm:110,length:8,division:1,samples:{},pattern:Array.from({length:32},()=>[])});
...
let stepPage=0,storageReadFailed=false;
...
async function decodeSample(k,target=p){const s=target.samples[k];if(!s)return;ensureAudio();const buf=await ctx.decodeAudioData(await s.blob.arrayBuffer());if(p!==target||p.samples[k]!==s)return;buffers[k]=buf;delete reversed[k];s.start=Math.max(0,Math.min(Number(s.start)||0,Math.max(0,buf.duration-.005)));s.end=Math.max(s.start+.001,Math.min(Number(s.end)||buf.duration,buf.duration));s.gain=Number.isFinite(s.gain)?s.gain:1;return buf}
function setStepPage(value){stepPage=Math.max(0,Math.min(3,Number(value)||0));$('stepPage').value=stepPage;renderSteps()}
$('stepPage').onchange=()=>setStepPage($('stepPage').value);
...
function renderSteps(){[...$('steps').children].forEach((b,i)=>{b.hidden=Math.floor(i/8)!==stepPage;b.disabled=i>=p.length;b.setAttribute('aria-pressed',chosen.has(i)||(Math.floor(i/8)===stepPage&&held.has('Digit'+(i%8+1))));b.querySelector('span').textContent=p.pattern[i].join('').toLowerCase()||'—';b.title='Step '+(i+1)+': '+(p.pattern[i].join(', ')||'empty')});$('meter').textContent='· '+(p.length*p.division/4)+' bars of 4/4'}
...
function trigger(k,when=ctx.currentTime){const s=p.samples[k],buf=buffers[k];if(!s)return;if(!buf){say(k+' audio unavailable · tap its pad to retry loading.');return;}const source=ctx.createBufferSource(),gain=ctx.createGain();const start=Math.max(0,Math.min(s.start||0,Math.max(0,buf.duration-.005))),end=Math.max(start+.001,Math.min(s.end||buf.duration,buf.duration));let data=buf,offset=start;if(s.reverse){if(!reversed[k]){const b=ctx.createBuffer(1,buf.length,buf.sampleRate);b.copyToChannel(Float32Array.from(buf.getChannelData(0)).reverse(),0);reversed[k]=b}data=reversed[k];offset=buf.duration-end}source.buffer=data;const rate=2**((s.pitch||0)/12);source.playbackRate.value=rate;gain.gain.value=s.gain;const effects=[];let tail=source;if(s.compression){const compressor=ctx.createDynamicsCompressor();compressor.threshold.value=-24;compressor.ratio.value=s.compression;tail.connect(compressor);tail=compressor;effects.push(compressor)}if((s.cutoff??20000)<20000){const filter=ctx.createBiquadFilter();filter.type='lowpass';filter.frequency.value=s.cutoff;tail.connect(filter);tail=filter;effects.push(filter)}if(s.pan){const pan=ctx.createStereoPanner();pan.pan.value=s.pan;tail.connect(pan);tail=pan;effects.push(pan)}tail.connect(gain).connect(master);voices.add(source);source.onended=()=>{voices.delete(source);source.disconnect();for(const effect of effects)effect.disconnect();gain.disconnect()};const length=Math.max(.001,end-start);source.start(when,Math.max(0,offset),length);flash(k,when,length/rate)}
...
async function toggle(){if(!ready){await audio();say('Loading saved samples · try again shortly.');return}if(playing){stop();return}await audio();playing=true;stepTimeline.length=0;origin=nextTime=ctx.currentTime+.03;nextStep=0;$('play').textContent=mobileUI?'stop':'stop [space]';schedule();timer=setInterval(schedule,25)}
async function perform(k){if(!ready)return;selected=k;render();if(held.has('Backspace')||held.has('Delete')){if(deleteMode==='off')deleteMode='bulk';deletePad(k);return}if(recordHeld||held.has('Backquote')){await startRecording(k);return}const steps=new Set(chosen);for(let i=0;i<p.length;i++)if(Math.floor(i/8)===stepPage&&held.has('Digit'+(i%8+1)))steps.add(i);if(steps.size){if(!p.samples[k]){say(emptyHint(k));return}for(const i of steps){if(i>=p.length)continue;const row=p.pattern[i],at=row.indexOf(k);at<0?row.push(k):row.splice(at,1)}save();renderSteps();say('Updated '+steps.size+' steps · release steps to play pads freely.');return}await audio();if(!buffers[k]&&p.samples[k]){try{await decodeSample(k)}catch{say(k+' audio could not load · import it again or restore a backup.');return}}if(!buffers[k]){say(emptyHint(k));return}trigger(k);if(playing&&$('overdub').checked){const candidates=[...stepTimeline,{step:nextStep,time:nextTime}];const step=candidates.reduce((a,b)=>Math.abs(a.time-ctx.currentTime)<Math.abs(b.time-ctx.currentTime)?a:b).step%p.length;if(!p.pattern[step].includes(k))p.pattern[step].push(k);save();renderSteps()}}
...
async function load(n){
 ...
 }catch(e){storageReadFailed=true;throw e}finally{ready=true;$('project').disabled=false;$('project').value=project;render()}
}
$('project').onchange=guard(async()=>{if(recorder||pending||importing){$('project').value=project;throw Error('Finish recording or importing before switching projects.')}await load(Number($('project').value))});
...
KeyU:['busOpen','click'],KeyO:['restore','click'],KeyY:['fxOpen','click'],Backspace:['clearPattern','click'],Delete:['clearPattern','click']
};
