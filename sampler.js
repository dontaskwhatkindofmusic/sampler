...
// Desktop Safari can change the hardware clock when its microphone opens.
// Use the same explicit rate for its output graph and capture request. Keep
// other browsers (including iOS/iPadOS) on their existing native-rate path.
function desktopSafari(){const ua=typeof navigator==='undefined'?'':navigator.userAgent||'';return /Macintosh/.test(ua)&&/Safari/.test(ua)&&!/Chrome|Chromium|Edg|OPR/.test(ua)&&!(navigator.maxTouchPoints>1)}
function audioOptions(){return {latencyHint:'interactive',...(desktopSafari()?{sampleRate:48000}:{})}}
...
async function requestMicrophone(){
 ...
 if(!microphoneRequest){setAudioSession('auto');microphoneRequest=navigator.mediaDevices.getUserMedia({audio:{channelCount:1,...(desktopSafari()?{sampleRate:{ideal:48000}}:{}),echoCancellation:false,noiseSuppression:false,autoGainControl:false}}).then(s=>{microphone=s;setAudioSession('play-and-record');return s}).finally(()=>microphoneRequest=null)}
...
const busDefaults=()=>({gain:.65,compressor:false,threshold:-24,ratio:12});
...
function ensureAudio(){if(ctx&&ctx.state!=='closed')return;ctx=new AudioContext(audioOptions());captureModule=null;master=ctx.createGain();busCompressor=ctx.createDynamicsCompressor();busDry=ctx.createGain();busWet=ctx.createGain();master.connect(busDry).connect(ctx.destination);master.connect(busCompressor).connect(busWet).connect(ctx.destination);applyBus()}
...
class HoldCapture extends AudioWorkletProcessor {
 constructor(){super();this.captureRate=sampleRate;this.waiting=true;this.capturing=false;this.done=false;this.chunks=[];this.count=0;this.history=[];this.historySize=0;this.meterFrames=0;this.enabled=false;
 this.port.onmessage=({data})=>{if(data.type==='configure'){this.threshold=data.threshold;this.limit=Math.floor(data.limit*this.captureRate);this.enabled=true}if(data.type==='stop')this.finish()};}
 finish(){if(this.done)return;this.done=true;const audio=new Float32Array(this.count);let at=0;for(const c of this.chunks){audio.set(c,at);at+=c.length}this.port.postMessage({type:'complete',audio,rate:this.captureRate},[audio.buffer]);this.chunks=[];this.history=[];}
 ...
 if(this.meterFrames>=this.captureRate/15){this.port.postMessage({type:'level',db});this.meterFrames=0}
 if(this.waiting){if(db>=this.threshold){this.waiting=false;this.capturing=true;for(const c of this.history){this.chunks.push(c);this.count+=c.length}this.history=[];this.port.postMessage({type:'started'});}else{this.history.push(block);this.historySize+=block.length;while(this.historySize>this.captureRate*.02&&this.history.length>1)this.historySize-=this.history.shift().length;return true}}
...
function wav(buffer){const rate=buffer.sampleRate,n=Math.min(buffer.length,Math.floor(rate*12)),data=new ArrayBuffer(44+n*2),v=new DataView(data);const str=(at,s)=>[...s].forEach((c,i)=>v.setUint8(at+i,c.charCodeAt(0)));str(0,'RIFF');v.setUint32(4,36+n*2,true);str(8,'WAVEfmt ');v.setUint32(16,16,true);v.setUint16(20,1,true);v.setUint16(22,1,true);v.setUint32(24,rate,true);v.setUint32(28,rate*2,true);v.setUint16(32,2,true);v.setUint16(34,16,true);str(36,'data');v.setUint32(40,n*2,true);const channels=Array.from({length:buffer.numberOfChannels},(_,i)=>buffer.getChannelData(i));for(let i=0;i<n;i++){let x=0;for(const c of channels)x+=c[i]/channels.length;v.setInt16(44+i*2,Math.max(-1,Math.min(1,x))*32767,true)}return new Blob([data],{type:'audio/wav'})}
